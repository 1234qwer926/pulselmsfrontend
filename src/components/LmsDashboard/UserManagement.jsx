  ...
  const openDeleteModal = (user) => {
    modals.openConfirmModal({
      ...
      children: (
        ...
        <Text size="sm">
          Are you sure you want to delete the mapping for <Text span fw={700}>{user.email}</Text>?
  ...
  return (
    <Container p="lg">
      <Modal
        ...
        centered
      >
        <UserMapping onSuccess={handleMapSuccess} onClose={() => setMapModalOpened(false)} />
      </Modal>
      ...
      {loading ? (
        <Text ta="center" c="dimmed">Loading users...</Text>
      ) : (
