...
export function FeaturesCards() {
  ...
  const features = mockdata.map((feature) => (
    <Card key={feature.title} shadow="md" radius="md" className={classes.card} padding="xl" bg={isDark ? 'dark.6' : 'white'}>
      <feature.icon size={50} stroke={1.5} color={isDark ? theme.colors.blue[3] : theme.colors.blue[6]} />
      <Text fz="lg" fw={500} className={classes.cardTitle} mt="md" c={isDark ? 'white' : 'black'}>
        {feature.title}
      </Text>
